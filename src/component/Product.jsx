...
export default function Product({id,title,price,discountPercentage,rating,brand,images}) {
  ...
  return (
    <div key={id} className='container border rounded-md shadow-md hover:scale-105 cursor-pointer'>
        <div>
        ...
        <div className='my-2'>
            <Link to={`/products/${id}`} className="bg-red px-2 py-1 m-1 text-blue rounded-md hover:text-white">Details</Link>
        </div>
    </div>
  )
}
